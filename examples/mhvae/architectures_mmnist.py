...
class my_input_encoder(BaseEncoder):
    ...
    def __init__(self):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(3,32,kernel_size=3, stride=1, padding=1, bias=True), nn.SiLU(),
            nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1, bias=True),nn.SiLU())
...
class bu_2(BaseEncoder):
    ...
    def forward(self, x):
        h = self.network(x)
        return ModelOutput(
            embedding = soft_clamp(self.mu(h)),
            log_covariance = soft_clamp(self.log_var(h))
        )
...
class prior_block(BaseEncoder):
    ...
    def forward(self, x):
        return ModelOutput(embedding = soft_clamp(self.mu(x)), log_covariance = soft_clamp(self.logvar(x)))
...
class posterior_block(BaseEncoder):
    ...
    def forward(self, x):
        h = self.network(x)
        return ModelOutput(embedding = soft_clamp(self.mu(h)), log_covariance = soft_clamp(self.logvar(h)))
