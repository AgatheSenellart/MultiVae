...
class EncoderImage(BaseEncoder):
    ...
    def __init__(self, latent_dim):
        ...
        self.var_layer = torch.nn.Sequential(torch.nn.Linear(512,latent_dim), torch.nn.Softplus())
    ...
    def forward(self,x):
        ...
        return ModelOutput(embedding=emb,
...
class ImageDecoder(BaseDecoder):
    ...
    def forward(self,z):
        ...
        return ModelOutput(
            reconstruction = h.reshape(shape)
        )
...
model = JMVAE(
    ...
    encoders = dict(images = EncoderImage(model_config.latent_dim),
                    labels=EncoderLabels(model_config.latent_dim)),
    decoders = dict(images = ImageDecoder(model_config.latent_dim),
