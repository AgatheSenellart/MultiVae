...
# Train a MAF Sampler
sampler_training_config = BaseTrainerConfig(per_device_train_batch_size=256, num_epochs=20, learning_rate=1e-3)
sampler_config = MAFSamplerConfig()
maf_sampler = MAFSampler(model)
maf_sampler.fit(train_data = train_data, eval_data = eval_data,training_config=sampler_training_config)
...
for sampler in samplers:
    ...
    module_eval = CoherenceEvaluator(model,classifiers,test_data, eval_config=config,sampler=sampler)
