...
from .base import IncompleteDataset, DatasetOutput
import os
import torch
import numpy as np
...
class MHD(IncompleteDataset):# pragma: no cover
    ...
    def __init__(self, 
        ...
        if not os.path.exists(self.data_file):
            if not download:
                raise RuntimeError(f'Dataset not found at path {datapath} and download is set to False'
                ...
                try :
                    ...
                    raise RuntimeError('gdown must be installed to download the dataset automatically.'
                                       'Install gdown with '
        ...
        self._s_data, self._i_data, self._t_data, self._a_data, self._traj_normalization, self._audio_normalization = torch.load(self.data_file)
        ...
        self.is_incomplete = sum([sum(missing_probabilities[s]) for s in missing_probabilities]) != 0
        ...
        if self.is_incomplete:
            ...
            for i,mod in enumerate(self.data):
                ...
                self.masks[mod] = torch.bernoulli(p,
                                                generator=torch.Generator().manual_seed(seed+i)).bool()
    ...
    def __download__(self,split,datapath): #pragram : no cover
        if split == 'train':
            gdown.download('https://docs.google.com/uc?export=download&id=1Tj1i-hXA0INQpU0jmuTMO4IwfDoGD2oV',
                                   output=datapath)
        else :
            gdown.download('https://docs.google.com/uc?export=download&id=1qiEjFNCFn1ws383pKmY3zJtm4JDymOU6',
                                   output=datapath)
