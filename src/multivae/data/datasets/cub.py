...
class CUBSentences(Dataset):
    ...
    def __init__(self, root_data_dir, split, output_type = 'one_hot', transform=None, **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        if self.output_type == 'one_hot':
            ...
            raise AttributeError("The output type should be either 'one_hot' or 'tokens' but is neither.")
...
class CUB(MultimodalBaseDataset):
    ...
    def __init__(self, path:str, split='train', 
        ...
        if split == 'eval':
            self.text_data = CUBSentences(path,'train',output_type=output_type,transform=transform_text, max_sequence_length=max_words_in_caption)
            self.image_data = datasets.ImageFolder(os.path.join(path, 'cub','train'), transform=transform_img)

        else :
            self.text_data = CUBSentences(path,split,output_type=output_type,transform=transform_text, max_sequence_length=max_words_in_caption)
            self.image_data = datasets.ImageFolder(os.path.join(path, 'cub',split), transform=transform_img)
    ...
    def check_or_download_data(self, data_path, download):
        ...
        if not os.path.exists(os.path.join(data_path, 'cub')):
            if download:
                ...
                logger.info(
                    f"Downloading the CUB dataset into {data_path}"
                )
