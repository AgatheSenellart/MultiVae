    ...
    def check_or_create_dataset(self,unimodal_paths, background_path, split):
        ...
        if not data_exists:
            if background_path is None:
                raise  ValueError("The provided path does not contain the dataset in the proper format"
        ...
        for digit in range(10):
            ...
            for m in range(self.num_modalities):
                ...
                for i, ix in enumerate(ixs_perm):
                    ...
                    new_img = self._add_background_image(background_images[m],
                        mnist.data[ix])
                    # save as png
                    filepath = os.path.join(self.save_path, "m%d/%d.%d.png" % (m, i, digit))
                    save_image(new_img, filepath)
                    # log the progress
                    cnt += 1
                    if cnt % 10000 == 0:
                        logger.info("Saved %d/%d images to %s" % (cnt, len(mnist)*self.num_modalities, self.save_path))
        assert cnt == len(mnist) * self.num_modalities

    
    def _add_background_image(self,background_image_pil, mnist_image_tensor,
            change_colors=False):

        # translate mnist image: downsamle digit and place it at a random location
        if self.translate is True:
            mnist_image_tensor_downsampled = \
                torch.nn.functional.interpolate(mnist_image_tensor.unsqueeze(0).float(), scale_factor=self.scale, mode="bilinear")
            ...
            mnist_image_tensor[:, x:x+int(28*(self.scale)), y:y+int(28*(self.scale))] = mnist_image_tensor_downsampled
