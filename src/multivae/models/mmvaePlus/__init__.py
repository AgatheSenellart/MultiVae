...
multiple latent spaces : :math:`z` is the latent code \textit{shared} accross modalities and ;math:`w_j` is private latent code of modality math:`j \in \[|1 , M\|]`. 
...
.. math::
    D^{\beta}_{\Phi,\Theta}(X,z^k, \tilde{w}_1^k, \tilde{w}_2^k,.., w_2^m, .., \tilde{w}_M^k) = \frac{p_{\theta_m}(x_m|z^k, w_m^k)(p(z^k)p(w_m^k))^{\beta}}{(q_{\phi_z}(z^k|X)q_{\phi_{w-m}}(w_m^k|x_m))^{\beta}}\prod_{n \neq m}p_{\theta_n}(x_n|z^k,\tilde{w}_n^k)
...
For cross modal reconstruction, only the shared latent code can be used which forces it to contain the information shared across modalities. 

.. note::
    For the partially observed case, that loss can be computed using only available sample instead of all modalities. 

    If we only observe modalities in :math:`S_{obs}(X)` the computed loss for sample :math:`X` becomes:
