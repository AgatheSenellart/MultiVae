...
class DCCA(Module):
    ...
    def set_networks(self, networks):
        ...
        assert(len(networks) == self.n_modalities, "The number of provided networks doesn't match the number of modalities.")
        
        for m in networks:
            if not isinstance(networks[m], BaseEncoder):
                raise AttributeError("The DCCA networks must be instances of pythae BaseEncoder class.")
            if networks[m].latent_dim != self.latent_dim:
                raise AttributeError("The DCCA networks must have the same latent dim as the DCCA model"
    ...
    def forward(self, inputs: MultimodalBaseDataset):
        ...
        if not all([k in self.networks for k in inputs.data]):
            raise AttributeError(f"Some inputs keys {inputs.data.keys()} are not known modalities."
