...
from .jnf_clip_config import JNFCLIPConfig
...
class JNFCLIP(BaseJointModel):
    ...
    Args:
        ...
        model_config (JNFCLIPConfig): An instance of JNFConfig in which any model's parameters is
            made available.
        ...
        CLIP_networks (Dict[str, BaseEncoder]) : A dictionary containing the networks to use in the DCCA module
    ...
    def __init__(
        ...
        CLIP_networks: Dict[str, BaseEncoder] = None,
        **kwargs,
    ):
        self.clip_config = CLIPConfig(
            joint_embedding_dim = model_config.embedding_clip_dim,
            n_modalities=model_config.n_modalities,
            weights=model_config.weights_clip,
        )

        if CLIP_networks is None:
            if model_config.input_dims is None:
                raise AttributeError(
                    "Please provide clip_networks or input_dims for the modalities in the model_config."
                ...
                CLIP_networks = BaseDictEncoders(
                    model_config.input_dims, model_config.embedding_clip_dim
                )
        else:
            model_config.custom_architectures.append("CLIP_networks")

        super().__init__(model_config, encoders, decoders, joint_encoder, **kwargs)

        # The default encoders for this model have (embedding_clip_dim, ) as input_size
        if encoders is None:
            encoders_input_dims = {
                k: (model_config.embedding_clip_dim,) for k in CLIP_networks
            }
            self.set_encoders(
        ...
        if joint_encoder is None:
            ...
            self.set_joint_encoder(
                MultipleHeadJointEncoder(CLIP_networks, model_config)
            )

        self.CLIP_module = CLIP(self.clip_config, CLIP_networks)
        self.CLIP_networks = self.CLIP_module.networks
        ...
        self.model_name = "JNFCLIP"
        self.warmup = model_config.warmup
        self.nb_epochs_clip = model_config.nb_epochs_clip
        self.reset_optimizer_epochs = [
            self.nb_epochs_clip,
            self.nb_epochs_clip + self.warmup,
        ]
    ...
    def _set_torch_no_grad_on_clip_module(self):
        self.CLIP_module.requires_grad_(False)

    def forward(self, inputs: MultimodalBaseDataset, **kwargs):
        epoch = kwargs.pop("epoch", 1)

        if epoch <= self.nb_epochs_clip:
            return self.CLIP_module(inputs)
        else:
            self._set_torch_no_grad_on_clip_module()
        ...
        if epoch <= self.warmup + self.nb_epochs_clip:
            return ModelOutput(
            ...
            for mod in self.encoders:
                clip_embed = self.CLIP_module.networks[mod](inputs.data[mod]).embedding


                mod_output = self.encoders[mod](clip_embed)
    ...
    def encode(
        ...
        **kwargs,
        ...
        if len(cond_mod) == self.n_modalities:
            ...
            cond_mod = cond_mod[0]
            clip_embed = self.CLIP_module.networks[cond_mod](
                inputs.data[cond_mod]
            ).embedding
            output = self.encoders[cond_mod](clip_embed)
            sample_shape = [] if N == 1 else [N]
