...
from multivae.models.nn.base_architectures import BaseJointEncoder, BaseConditionalDecoder, BaseEncoder
from multivae.data.datasets.base import MultimodalBaseDataset
from .cvae_config import CVAEConfig
import torch.distributions as dist
import torch
from multivae.models.base.base_utils import set_decoder_dist
from multivae.models.nn.default_architectures import Encoder_VAE_MLP, ConditionalDecoder_MLP, MultipleHeadJointEncoder, BaseDictEncoders
...
class CVAE(BaseModel):
    ...
    def __init__(self, model_config, encoder=None, decoder=None,prior_network=None):
        ...
        self.set_decoder_dist(model_config.decoder_dist, model_config.decoder_dist_params)
    ...
    def set_decoder(self, decoder, model_config):
        ...
        if not isinstance(decoder, BaseConditionalDecoder):
            raise ValueError("The decoder must be an instance of BaseConditionalDecoder")
    ...
    def set_prior_network(self, prior_network):
        ...
        if prior_network is None:
            self.prior_network = None # the prior will be fixed to a standard normal distribution
    ...
    def default_encoder(self, model_config):
        ...
        if model_config.input_dims is None:
            raise AttributeError('No encoder was provided but model_config.input_dims is None',
                                 'Please provide the input_dims of the model or an encoder architecture')
        
        return MultipleHeadJointEncoder(
            dict_encoders= BaseDictEncoders(model_config.input_dims, model_config.latent_dim),
            args=model_config,
            hidden_dim=512,
            n_hidden_layers=2
        )
    ...
    def default_decoder(self, model_config):
        ...
        if model_config.input_dims is None:
            raise AttributeError('No decoder was provided but model_config.input_dims is None',
        ...
        return ConditionalDecoder_MLP(
            ...
            conditioning_data_dim=model_config.input_dims[model_config.conditioning_modality]
        )        
    ...
    def forward(self, inputs: MultimodalBaseDataset, **kwargs) -> ModelOutput:
        ...
        recon_loss = -self.recon_log_prob(recon, inputs.data[self.main_modality]).mean(0).sum()
        
        # Compute the KL divergence between the posterior and the prior
        kl_div = self.kl_divergence(embedding, log_var, prior_mean, prior_log_var).mean(0).sum()
    ...
    def kl_divergence(self, mean, log_var, prior_mean, prior_log_var):
        ...
        kl = 0.5* (prior_log_var - log_var -1 + torch.exp(log_var - prior_log_var) + (mean - prior_mean)**2) / torch.exp(prior_log_var)
    ...
    def decode(self, embedding:ModelOutput, **kwargs) -> ModelOutput:
        ...
        if len(z.shape)==3:
            ...
            output.reconstruction = output.reconstruction.reshape(N,l,*output.reconstruction.shape[1:])
            return output
        else :
    ...
    def generate_from_prior(self, cond_mod_data:torch.Tensor, N:int = 1, **kwargs) -> ModelOutput:
        ...
        sample_shape = [] if N == 1 else [N]
        z = dist.Normal(prior_mean, torch.exp(0.5*prior_log_var)).rsample(sample_shape)
    ...
    def predict(self, 
        ...
        if len(cond_mod) == 1:
            ...
            if cond_mod[0] == self.conditioning_modality:
                embeddings = self.generate_from_prior(cond_mod_data=inputs.data[self.conditioning_modality],N=N, **kwargs)
            elif cond_mod[0] == self.main_modality:
                embeddings = self.encode(inputs,N, **kwargs)
            else:
                raise ValueError('The conditioning modality must be one of the modalities of the model'
                                 '. You provided {}'.format(cond_mod[0]))
        elif len(cond_mod) == 2:
            
            if self.conditioning_modality not in cond_mod or self.main_modality not in cond_mod:
                raise ValueError('One of the modalities in cond_mod is not part of the model'
                                 'cond_mod : {}'.format(cond_mod))
            embeddings = self.encode(inputs,N, **kwargs)
        
        else:
            raise ValueError('The length of cond_mod must be 1 or 2 since the model is a CVAE')
