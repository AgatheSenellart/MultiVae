...
class BaseTrainer:
    ...
    def __init__(
        ...
        if training_config is None:
            if checkpoint is None:
                ...
                training_config = BaseTrainerConfig.from_json_file(os.path.join(checkpoint,'training_config.json'))
    ...
    def resume_training(self, checkpoint):
        ...
        # set optimizer
        self.set_optimizer()
        self.optimizer.load_state_dict(torch.load(os.path.join(checkpoint,'optimizer.pt'),map_location=self.device))

        # set scheduler
        self.set_scheduler()
        if self.scheduler is not None:
            self.scheduler.load_state_dict(torch.load(os.path.join(checkpoint,'scheduler.pt') ,map_location=self.device))
    ...
    def train(self, log_output_dir: str = None):
        ...
        for epoch in range(self.trained_epochs+1, self.training_config.num_epochs + 1):
    ...
    def save_checkpoint(self, model: BaseMultiVAE, dir_path, epoch: int):
        ...
        self.callback_handler.on_save_checkpoint(self.training_config,checkpoint_dir=checkpoint_dir)
