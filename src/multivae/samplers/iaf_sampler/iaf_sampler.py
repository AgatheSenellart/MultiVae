...
class IAFSampler(BaseSampler):
    ...
    def fit(
        ...
        for m in train_data: # number of latent_spaces
            ...
            train_dataset = BaseDataset(data = train_data[m], labels=torch.zeros((len(train_data[m]),)))
            eval_dataset = None if eval_data is None else BaseDataset(data = eval_data[m], labels=torch.zeros((len(eval_data[m]),)))
