...
class Clustering(Evaluator):
    ...
    def __init__(
        ...
        if eval_config.clustering_method == 'kmeans':
            try : 
                ...
                raise ModuleNotFoundError("scikit-learn must be installed to perform clustering.")
            self.clustering = KMeans(n_clusters=eval_config.n_clusters,
                                     max_iter=300
                                     )
    ...
    def fit_clustering(self, mods = 'all'):
        ...
        labels = []
        for inputs in dl:
            if self.num_samples_for_fit is not None and n_samples > self.num_samples_for_fit:
                break
            inputs = set_inputs_to_device(inputs,self.device)
            with torch.no_grad():
                list_z.append(self.model.encode(inputs, mods, return_mean=self.use_mean).z)
