        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                if self.flags.factorized_representation:
                    ...
                    m_s_embeddings = None;
                m_rec = self.lhoods[m_key](self.decoders[m_key](class_embeddings).reconstruction, 0.75);
...
from multivae.models.nn.default_architectures import ModelOutput, Encoder_VAE_MLP, Decoder_AE_MLP, BaseAEConfig
...
modalities = dict(
    m0 = ModelOutput(encoder = Encoder_VAE_MLP(BaseAEConfig(input_dim = (2,), latent_dim = 10)),
                     decoder = Decoder_AE_MLP(BaseAEConfig(input_dim = (2,), latent_dim = 10)),
                     likelihood = torch.distributions.Laplace,
                     name='m0'),
    m1 = ModelOutput(encoder = Encoder_VAE_MLP(BaseAEConfig(input_dim = (3,), latent_dim = 10)),
...
data = dict(m0 = torch.tensor([[1,2],[3,4]]).float(), m1 = torch.tensor([[1,2,5],[3,4,6]]).float())
...
encoders = dict(m0 = modalities['m0'].encoder,
                m1 = modalities['m1'].encoder)

decoders = dict(m0 = modalities['m0'].decoder,
                m1 = modalities['m1'].decoder)
...
def calc_log_probs(flags, modalities, result, batch):
    ...
    for m, m_key in enumerate(mods.keys()):
        mod = mods[m_key]
        log_probs[mod.name] = -calc_log_prob(result['rec'][mod.name],
                                                 batch[mod.name],
                                                 flags.batch_size);
