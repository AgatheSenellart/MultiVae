...
class TestMVAE:
    ...
    def model_config(self, request):
        """Create the model configuration for testing"""
    ...
    def test_encode(self, model, dataset):
        ...
        for return_mean in [True, False]:
            ...
            # Encode the entire dataset, generate one latent code/datapoint
            embeddings = model.encode(
                dataset, cond_mod=["mod2"], return_mean=return_mean
            ).z
            assert embeddings.shape == (len(dataset), 5)

            # Encode the whole dataset, generate 10 latent samples per datapoint
            embeddings = model.encode(
