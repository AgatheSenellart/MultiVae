...
class Test_BaseMultiVAE:
    ...
    def input_model(self):
        ...
        encoders = dict(
            ...
            mod2 = Encoder_AE_MNIST(config)
        )
        decoders = dict(
            mod1 = Decoder_AE_MLP(config),
            mod2 = Decoder_AE_MNIST(config)
        )
    ...
    def test(self, input_model):
        ...
        assert isinstance(model.encoders['mod2'],Encoder_AE_MNIST)
        assert type(model.decoders)==dict
        assert isinstance(model.decoders['mod1'],Decoder_AE_MLP)
        assert isinstance(model.decoders['mod2'],Decoder_AE_MNIST)
