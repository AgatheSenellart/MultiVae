...
from multivae.data.datasets.utils import adapt_shape, ResampleDataset
...
class TestUtils:
    ...
    def test_resample_dataset(self, dummy_data):
        ...
        assert all([dataset[torch.tensor(0)].data[m].shape == dummy_data[m].shape[1:] for m in dummy_data.keys()])
